...
const handleSearch = async () => {
  ...
  formData.append('time_start', dateRange.from.toISOString());
  formData.append('time_end', dateRange.to.toISOString());
  formData.append('latitude', selectedLocation.latitude.toString());
  formData.append('longitude', selectedLocation.longitude.toString());

  try {
    const res = await fetch('http://10.231.243.217:5001/upload/lost', {
      method: 'POST',
      body: formData,
      headers: {
